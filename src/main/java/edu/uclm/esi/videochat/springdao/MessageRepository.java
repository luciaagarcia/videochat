...
public interface MessageRepository extends CrudRepository <Message, String> {
    @Query(value = "SELECT date, message, recipient, sender FROM message WHERE (recipient=:recipient AND sender=:sender) OR (recipient=:sender AND sender=:recipient) ORDER BY date", nativeQuery = true)
